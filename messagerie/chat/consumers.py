...
class GetWebSocket(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        if action == 'connect':
            ...
            await self.manageChat(data['uuid'], usernames=data['username'], flag=True)
        
        elif action == 'deleteGr':
            await self.manageChat(data['uuid'], flag=True)


    async def handleMsg(self, text=None, type='normal', username=None, action='message'):
        if action == 'connected' : chat = None 
    ...
    async def manageChat(self, uuid, namegr=None, usernames=None, flag=False):
    ...
    async def sendStatusUser(self, all=None):
        ...
        if all:
            ...
            chats = await db_query(lambda x: [str(y) for y in x])(chats)
            for chat in chats:
                if self.user.username in self.connectedUsers:
                    text = "En ligne"
                else:
                    text = str(self.user.last_login)[:16]
                user = getUser2((chat).split(' - '), self.user.username)
            ...
            if user in self.connectedUsers:
                await self.send(json.dumps({'action':'connected', 'status': "En ligne"}))
            else:
                user = await self.get_user(user)
                await self.send(json.dumps({'action':'connected', 'status': str(user.last_login)[:16]}))
