...
const handleMsg = () => {
  document.getElementById('inputMsgBtn').addEventListener('click', function(e) {
    ...
    let input = document.getElementById('id_text')
    if (input.value.trim() != "") {
      data = {
        'action':'message',
        'text': input.value,
...
alert.addEventListener('click', (e) => {
  let uuid = e.target.getAttribute('id').split('id')[1];
  postChangeChat(uuid);
})
...
function userStatus(status, uuid) {
  if (document.querySelector('.message #id' + uuid)){
...
function postDeleteChat(uuid) {
  websocket.send(JSON.stringify({'action':'disconnect', 'uuid':uuid}));
  getChats();
