...
setupChat = (function() {
  ...
  function autoSizeTextarea() {
    msgClass.querySelector('#id_text').addEventListener('input', (e) => {
  ...
  function switchTabMobileScreen(switchTab) {  
    if (!mediaScreen.matches && switchTab) {
        if (chatList.checkVisibility()){
          chatList.parentElement.style.display = 'none';
        } else {
          chatList.parentElement.style.display = 'block';
        }
        msgClass.querySelector('#backBtn').addEventListener('click', function(e) {
  ...
  function sendingMsg() {
    msgClass.querySelector('#inputMsgBtn').addEventListener('click', function(e) {
...
function createChatOperations(data) {
  ...
  createChat.querySelectorAll('.user').forEach(element => {
    element.addEventListener('click', (e) => {
      websocketSend(action='createChat', usernames=e.target.innerHTML);
    })
  })
  ...
  submitGrBtn.addEventListener('click', () => {
    if (createGrInput.value != '' && usersSelected.length != 0) {
      websocketSend(action='createGr', usernames=usersSelected,
      text=createGrInput.value )} 
  })
